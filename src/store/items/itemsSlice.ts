...
const initialState: ItemsSliceState = {
  ...
  currentCat: '',
  brands: [
    ...[...new Set([...getItemsFromAdmin()].map(i => i.brand))].sort((a, b) =>
      a.localeCompare(b)
    ),
  ],
  sellers: [
    ...[...new Set([...getItemsFromAdmin()].map(i => i.seller))].sort((a, b) =>
      a.localeCompare(b)
    ),
  ],
};
...
const itemsSlice = createSlice({
  ...
  reducers: {
    updateItems(state) {
      ...
      state.brands = [
        ...[...new Set([...getItemsFromAdmin()].map(i => i.brand))].sort(
          (a, b) => a.localeCompare(b)
        ),
      ];
      state.sellers = [
        ...[...new Set([...getItemsFromAdmin()].map(i => i.seller))].sort(
          (a, b) => a.localeCompare(b)
        ),
      ];
    },
    ...
    removeFromLocalStorage(state, action: PayloadAction<string>) {
      state.items = [...getItemsFromAdmin()].filter(
      ...
      if (state.currentCat.length) {
        ...
        state.items = [...getItemsFromAdmin()].filter((i: ItemsType) => {
          return (
            i.code ===
            (findItem
              ? findItem.itemsCodes.find(item => item === i.code)
              : null)
          );
        });
      } else {
    ...
    filterItems(state) {
      ...
      if (state.filters.length > 1) {
        [...state.filters].splice(1, state.filters.length - 1).forEach(f => {
          let g = [...getItemsFromAdmin()].filter(
            (item: ItemsType) => item[f.key] === f.value
          );
          if (g) temp = [...temp, ...g];
        });
        ...
        let filtersLength =
          [...new Set([...[...state.filters].map(i => i.key)])].length - 2;

        for (let i = 0; i < temp.length; i++) {
          if (
            temp[i + filtersLength] &&
            temp[i].code === temp[i + filtersLength].code
          ) {
            res = [...res, temp[i]];
          }
        }
