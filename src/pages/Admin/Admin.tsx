...
const Admin = () => {
    ...
    if (event.target.checked && newCats) {
      newCats = [
        ...[...newCats].filter(item => item.name !== event.target.value),
        {
          ...
          itemsCodes: findItemNew
            ? [...findItemNew.itemsCodes, String(id)]
            : [String(id)],
        },
      ];
  ...
  return (
    <div className={styles.admin}>
      ...
      <br />
      <button
        className={`${styles.btn__open} ${form ? styles.open : styles.close}`}
      ...
      {form && (
        <div>
          <form>
            ...
            <button
              type={'button'}
              className={`${styles.btn__open} ${styles.close}`}
              onClick={submit}
            >
              Add
            </button>
      ...
      <div className={styles.admin__grid}>
        {d && JSON.parse(d).length ? (
          [...JSON.parse(d)]
            .sort((a, b) => a.code - b.code)
            .map((i: ItemsType) => <AdminCard i={i} key={i.code}></AdminCard>)
        ) : (
          <div>No items at localstorage</div>
        )}
      </div>
    </div>
  );
