    ...
    if (event.target.checked && newCats) {
      ...
      newCats = [
        ...[...newCats].filter(item => item.name !== event.target.value),
        {
          name: findItemNew ? findItemNew.name : '',
          itemsCodes: findItem
            ? [...findItem.itemsCodes.filter(c => c !== i.code)]
            : [],
        },
      ];
    }
        ...
        <div className={styles.admin__card__inner} key={i.code}>
          ...
          <div className={styles.add}>
            ...
            <p>
              {' '}
              Штрихкод:
              <span className={styles.item__info}>{i.code}</span>
            </p>
                ...
                .map((c: Categories) => (
                  <span key={c.name}>
                    <input
                      ...
                      value={c.name}
                      checked={
                        c.itemsCodes.find(item => item === i.code) === i.code
                      }
                      onChange={changeCat}
                    />{' '}
                    {c.name}
          ...
          <div className={styles.text__container}>
            ...
            <Link to={PRODUCT_ROUTE + '/' + i.code}>
              <p className={styles.name}>
                {' '}
                <strong>{i.brand.toUpperCase()}</strong> {i.name}{' '}
              </p>
            </Link>
            <div className={styles.info__container}>
              <p>
                Штрихкод: <span className={styles.item__info}>{i.code}</span>
              </p>

              <p>
                Производитель:{' '}
                <span className={styles.item__info}>{i.seller}</span>
              </p>
