...
export const calcTotalAmount = (arr: any[]): number =>
  arr.reduce(a => a + 1, 0);

export const calcCartAmount = (arr: CartItem[]): number =>
  arr.reduce((a, b: CartItem) => a + b.count, 0);
...
export const filterPrice = (arr: ItemsType[], filters: iFilters[]) => {
  return arr.filter(
    (item: ItemsType) =>
      item.price >= +filters[0].value[0] && item.price <= +filters[0].value[1]
  );
};
